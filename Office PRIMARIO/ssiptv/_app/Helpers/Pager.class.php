  ...
  */
  ...
  private function getSyntax() {
    ...
    if ($this->Rows > $this->Limit):
      ...
      $this->Paginator .= "<li><a title=\"{$this->First}\" class='".($this->Href ? "" : "j_btn_pag")."' pag='1' href=\"{$this->Link}1\">{$this->First}</a></li>";

      for ($iPag = $this->Page - $MaxLinks; $iPag <= $this->Page - 1; $iPag ++):
        if ($iPag >= 1):
          $this->Paginator .= "<li><a title=\"Página {$iPag}\" class='".($this->Href ? "" : "j_btn_pag")."' pag='{$iPag}' href=\"{$this->Link}{$iPag}\">{$iPag}</a></li>";
        endif;
      endfor;
      ...
      for ($dPag = $this->Page + 1; $dPag <= $this->Page + $MaxLinks; $dPag ++):
        if ($dPag <= $Paginas):
          $this->Paginator .= "<li><a title=\"Página {$dPag}\" class='".($this->Href ? "" : "j_btn_pag")."' pag='{$dPag}' href=\"{$this->Link}{$dPag}\">{$dPag}</a></li>";
        endif;
      endfor;

      $this->Paginator .= "<li><a title=\"{$this->Last}\" class='".($this->Href ? "" : "j_btn_pag")."' pag='{$Paginas}' href=\"{$this->Link}{$Paginas}\">{$this->Last}</a></li>";
      $this->Paginator .= "</ul>";
    endif;
  }
